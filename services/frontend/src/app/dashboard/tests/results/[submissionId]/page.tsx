...
export default async function SubmissionResultPage({ params }: SubmissionResultPageProps) {
  ...
  const submissionResult = await getSubmissionResultAction(submissionId);
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6 mb-8">
        ...
        <div className="mt-6 pt-6 border-t border-gray-200">
          <p className="text-sm text-gray-600">
            Submitted on: {new Date(submission.submitted_at * 1000).toLocaleString()}
          </p>
        </div>
      </div>
        ...
        {questions.map((questionResult, index) => (
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {[
                { index: 0, label: 'A', text: questionResult.question.option_a },
                { index: 1, label: 'B', text: questionResult.question.option_b },
                { index: 2, label: 'C', text: questionResult.question.option_c },
                { index: 3, label: 'D', text: questionResult.question.option_d },
              ].map((option) => {
                const isSelected = questionResult.selected_answer === option.index;
                const isCorrect = questionResult.question.correct_answer === option.index;
