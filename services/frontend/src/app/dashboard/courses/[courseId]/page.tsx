...
export default async function StudentCoursePage({ params }: StudentCoursePageProps) {
  ...
  const sectionsWithArticles = await Promise.all(
    sections.map(async (section) => {
      const articlesResult = await text.listArticles({ section_id: section.id });
      return {
        ...section,
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            ...
            <div className="flex flex-wrap gap-4 text-sm text-gray-500">
              ...
              <div>
                <strong>Articles:</strong> {sectionsWithArticles.reduce((total, section) => total + section.articles.length, 0)}
              </div>
              <div>
              ...
              <div key={section.id} className="bg-white rounded-lg border shadow-sm overflow-hidden">
                ...
                <div className="p-6">
                  {section.articles.length === 0 ? (
                    <p className="text-gray-500 italic">No articles in this section yet.</p>
                  ) : (
