...
import { Save, RotateCcw, CheckCircle, XCircle, Clock } from 'lucide-react';
...
export function CodeEditor({ exerciseId, initialCode }: CodeEditorProps) {
    ...
    try {
      ...
      if (result?.success) {
        ...
        if (isSuccessful) {
          ...
          toast.success(
            'Solution submitted, but some tests failed. Keep trying!',
          );
        }
      } else {
  ...
  const handleReset = () => {
    if (
      confirm(
        'Are you sure you want to reset your code? This will restore the initial template.',
      )
    ) {
      setCode(initialCode);
      setResults([]);
                ...
                >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs">
                      ...
                      {result.actual && (
                        <div>
                          ...
                          <pre className="bg-white p-2 rounded border text-red-600">
                            {result.actual}
                          </pre>
                        </div>
                      )}
                  ...
                  {result.error && (
                    <div className="mt-2">
                      ...
                      <pre className="bg-white p-2 rounded border text-red-600 text-xs">
                        {result.error}
                      </pre>
                    </div>
                  )}
